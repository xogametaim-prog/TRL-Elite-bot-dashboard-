/**
 * Bot Version: 9.1.0v (Pure UNO Engine - Fixed Syntax Edition)
 * Developer: ta_im1 | Team: TRL for development
 * Platform: Optimized for Mobile (Pydroid 3 / Replit / Render)
 */
...
function generateDeck() {
    ...
    for (let color of CARD_COLORS) {
        for (let value of CARD_VALUES) {
            ...
            if (value !== '0') deck.push({ color, value, label: `${color} [${value}]` }); 
        }
    }
...
client.once('ready', async () => {
    ...
    // تصليح الأقواس هنا بالملي ومسح القوس الزائد اللي سبب الأيرور
    const commands = [
    ...
    try {
        ...
        console.log(`[SYSTEM] Global commands loaded successfully!`);
    } catch (e) { 
        console.error(e); 
    }
});
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.customId === 'uno_show_my_cards') {
        if (!unoGame) return interaction.reply({ content: '❌ Gen انتهى أو غير موجود حالياً.', ephemeral: true });
