...
# ========== قائمة الصفحة الحالية للسوق السوداء ==========
الصفحة_الحالية = 1
...
async def الاغنياء(التفاعل: discord.Interaction):
    ...
    for i, (المعرف, عملات) in enumerate(مرتبة):
        ...
        الوصف += f"{i+1}. **{المستخدم.display_name if المستخدم else 'مجهول'}** — {عملات} 🪙\n"
    تضمين = discord.Embed(title="🏆 قائمة الأغنياء", description=الوصف, color=0xFFD700)
    await التفاعل.response.send_message(embed=تضمين)

# ----- المتجر العادي -----
@bot.tree.command(name="المتجر", description="عرض المتجر العادي")
...
# ----- السوق السوداء -----
@bot.tree.command(name="بلاك_ماركت", description="عرض السوق السوداء (50 سلعة)")
async def بلاك_ماركت(التفاعل: discord.Interaction):
    global الصفحة_الحالية
    العناصر = await احصل_على_سلع_السوق_السوداء(الصفحة_الحالية)
    تضمين = discord.Embed(title=f"🔫 السوق السوداء - الصفحة {الصفحة_الحالية}/5", color=0xFF0000)
    for عنصر in العناصر:
        تضمين.add_field(name=f"{عنصر['id']}. {عنصر['name']}", value=f"🪙 {عنصر['coinPrice']} | 💎 {عنصر['creditPrice']}", inline=True)
    ...
    async def رد_السابق(تفاعل_الزر):
        nonlocal الصفحة_الحالية
        if الصفحة_الحالية > 1:
            الصفحة_الحالية -= 1
            عناصر_جديدة = await احصل_على_سلع_السوق_السوداء(الصفحة_الحالية)
            تضمين_جديد = discord.Embed(title=f"🔫 السوق السوداء - الصفحة {الصفحة_الحالية}/5", color=0xFF0000)
            for عنصر in عناصر_جديدة:
                تضمين_جديد.add_field(name=f"{عنصر['id']}. {عنصر['name']}", value=f"🪙 {عنصر['coinPrice']} | 💎 {عنصر['creditPrice']}", inline=True)
            await تفاعل_الزر.response.edit_message(embed=تضمين_جديد)
        else:
            await تفاعل_الزر.response.send_message("أنت في الصفحة الأولى", ephemeral=True)
    
    async def رد_التالي(تفاعل_الزر):
        nonlocal الصفحة_الحالية
        if الصفحة_الحالية < 5:
            الصفحة_الحالية += 1
            عناصر_جديدة = await احصل_على_سلع_السوق_السوداء(الصفحة_الحالية)
            ...
            await تفاعل_الزر.response.send_message("أنت في الصفحة الخامسة", ephemeral=True)
    
    زر_السابق.callback = رد_السابق
    زر_التالي.callback = رد_التالي
    العرض.add_item(زر_السابق)
    العرض.add_item(زر_التالي)
    
    await التفاعل.response.send_message(embed=تضمين, view=العرض)

@bot.tree.command(name="شراء_بلاك", description="شراء سلعة من السوق السوداء")
@app_commands.choices(العملة=[app_commands.Choice(name="عملات", value="coins"), app_commands.Choice(name="رصيد مميز", value="credits")])
...
@bot.tree.command(name="دخول_فريق", description="اختيار فريق من القائمة")
async def دخول_فريق(التفاعل: discord.Interaction):
    ...
    for فريق in الفرق:
        ...
        القائمة.add_option(label=فريق[2], description=f"بواسطة {المستخدم.display_name if المستخدم else 'مجهول'}", value=f"{فريق[0]}|{فريق[1]}")
...
# ----- السرقة -----
@bot.tree.command(name="سرقة", description="سرقة أموال من فريق خصم")
...
# ----- كتاب التخفي -----
@bot.tree.command(name="تخفي", description="إخفاء فريقك 30 دقيقة")
...
# ----- المهام -----
@bot.tree.command(name="مهامي", description="عرض مهامك")
...
@bot.tree.command(name="تسليم_مهمة", description="تسليم مهمة مكتملة")
...
@bot.tree.command(name="اعطاء_فلوس", description="[أونر] إعطاء عملات للاعب")
...
@bot.tree.command(name="حذف_فريق", description="[أونر] حذف فريق مخالف")
...
@bot.tree.command(name="اذاعة", description="[أونر] إرسال رسالة Embed للجميع")
