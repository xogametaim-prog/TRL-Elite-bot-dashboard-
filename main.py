...
def handle_exception(exc_type, exc_value, exc_traceback):
    logger.error("خطأ كارثي أثناء التشغيل:", exc_info=(exc_type, exc_value, exc_traceback))
...
if التوكن is None:
    logger.error("❌ التوكن غير موجود في متغيرات البيئة")
    sys.exit(1)
...
# ========== كلاس EmbedHelper الموحد ==========
class EmbedHelper:
    @staticmethod
    def create(title=None, description=None, color=None, image_url=None, fields=None, footer_text=None, author_name=None, author_icon=None, thumbnail_url=None):
        try:
            if color is None:
                color = 0x3498db
            ...
            logger.error(f"❌ خطأ في EmbedHelper.create: {e}")
    ...
    @staticmethod
    async def send(interaction_or_channel, title=None, description=None, color=None, image_url=None, fields=None, footer_text=None, is_ephemeral=False, view=None, author_name=None, author_icon=None, thumbnail_url=None):
        try:
            ...
            logger.error(f"❌ خطأ في EmbedHelper.send: {e}")
            try:
                fallback_embed = EmbedHelper.create(
                    description="حدث خطأ أثناء إرسال الرسالة. يرجى إبلاغ الإدارة.",
                    color=0xFF0000
                )
                if hasattr(interaction_or_channel, 'response'):
                    if interaction_or_channel.response.is_done():
                ...
                logger.error("❌ فشل إرسال رسالة الخطأ الاحتياطية")
            return None
...
async def حفظ_سجل_الروليت(guild_id, user_id, user_name, result):
    try:
        ...
        logger.error(f"❌ خطأ في حفظ سجل الروليت: {e}")
...
async def تحديث_نقاط_اللعبة(guild_id, user_id, user_name, points=1):
    try:
        ...
        logger.error(f"❌ خطأ في تحديث نقاط اللعبة: {e}")
...
async def الحصول_على_نقاط_اللاعب(guild_id, user_id):
    try:
        ...
        logger.error(f"❌ خطأ في الحصول على نقاط اللاعب: {e}")
        return {"points": 0, "games_won": 0}
...
async def تحديث_الدعوات(guild):
    try:
        ...
        logger.error(f"❌ خطأ في تحديث الدعوات: {e}")
...
async def تسجيل_نشاط_القناة(guild_id, channel_id):
    try:
        ...
        logger.error(f"❌ خطأ في تسجيل نشاط القناة: {e}")
...
class RouletteGame:
    ...
    async def start_game(self, interaction_or_ctx, is_slash=False):
        try:
            ...
            waiting_embed = game.get_waiting_embed()
            
            if is_interaction:
                await EmbedHelper.send(
                    interaction_or_ctx,
                    title=waiting_embed.title,
                    description=waiting_embed.description,
                    color=waiting_embed.color.value,
                    image_url=صورة_الروليت_الانتظار,
                    fields=[{"name": f.name, "value": f.value, "inline": f.inline} for f in waiting_embed.fields],
                    footer_text=waiting_embed.footer.text,
                    view=game
                )
                message = await interaction_or_ctx.original_response()
            else:
                message = await interaction_or_ctx.channel.send(embed=waiting_embed, view=game)
            
            game.message = message
            await game.start_countdown()
        except Exception as e:
            logger.error(f"❌ خطأ في بدء لعبة الروليت: {e}")
...
class RouletteGameSession(discord.ui.View):
    ...
    def get_waiting_embed(self):
        ...
        return EmbedHelper.create(
            ...
            color=0xFF6600,
            image_url=صورة_الروليت_الانتظار,
            fields=[{"name": "⏰ الوقت المتبقي", "value": "15 ثانية", "inline": True}],
            footer_text=f"بواسطة {self.bot.get_user(self.host_id).display_name if self.bot.get_user(self.host_id) else 'المشرف'}"
        )
    ...
    def get_turn_embed(self):
        ...
        return EmbedHelper.create(
            ...
            image_url=صورة_الروليت_الانتظار,
            fields=[{"name": "⏰ الوقت المتبقي", "value": "15 ثانية", "inline": True}],
            footer_text=f"بواسطة {current_player.user_name}"
        )
    
    def get_result_embed(self, kicked_player, kicked_by=None):
        players_list = "\n".join([f"{i+1}. <@{p.user_id}> {'❤️' if p.is_alive else '💀'}" for i, p in enumerate(self.players)])
        description = f"**{kicked_by.user_name} قام بطرد {kicked_player.user_name}!**\n\n{len([p for p in self.players if p.is_alive])} لاعب متبقي." if kicked_by else f"**{kicked_player.user_name} تم طرده تلقائياً لعدم الرد في الوقت المحدد!**\n\n{len([p for p in self.players if p.is_alive])} لاعب متبقي."
        return EmbedHelper.create(
            title="🎡 نتيجة الروليت",
            description=description,
            color=0xFF0000,
            image_url=صورة_الروليت_النتيجة,
            fields=[{"name": "👥 اللاعبين المتبقيين", "value": players_list, "inline": False}]
        )
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            if not self.game_active:
                await EmbedHelper.send(interaction, description="❌ اللعبة انتهت!", color=0xFF0000, is_ephemeral=True)
                return
            
            if any(p.user_id == interaction.user.id for p in self.players):
                await EmbedHelper.send(interaction, description="❌ أنت مشترك بالفعل!", color=0xFF0000, is_ephemeral=True)
                return
            
            self.players.append(GamePlayer(interaction.user.id, interaction.user.display_name))
            await EmbedHelper.send(interaction, description=f"✅ {interaction.user.mention} انضم إلى الروليت!", color=0x00FF00, is_ephemeral=True)
            
            if self.message:
                await self.message.edit(embed=self.get_waiting_embed(), view=self)
        except Exception as e:
            logger.error(f"❌ خطأ في زر الانضمام: {e}")
    ...
    async def leave_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not player:
                await EmbedHelper.send(interaction, description="❌ لست مشتركاً في هذه اللعبة!", color=0xFF0000, is_ephemeral=True)
                return
            
            self.players.remove(player)
            await EmbedHelper.send(interaction, description=f"✅ {interaction.user.mention} غادر الروليت!", color=0x00FF00, is_ephemeral=True)
            
            if self.message:
                await self.message.edit(embed=self.get_waiting_embed(), view=self)
        except Exception as e:
            logger.error(f"❌ خطأ في زر المغادرة: {e}")
    ...
    async def start_countdown(self):
        try:
            ...
            logger.error(f"❌ خطأ في العد التنازلي: {e}")
    ...
    async def start_turn(self):
        try:
            ...
            logger.error(f"❌ خطأ في بدء الدور: {e}")
    ...
    async def kick_player(self, interaction, target_player, kicker_player=None):
        try:
            ...
            if self.waiting_for_action == False:
                await EmbedHelper.send(interaction, description="❌ ليس دورك الآن!", color=0xFF0000, is_ephemeral=True)
                return
            ...
            if kicker_player:
                if interaction.user.id != kicker_player.user_id:
                    await EmbedHelper.send(interaction, description="❌ ليس دورك!", color=0xFF0000, is_ephemeral=True)
                    return
            else:
            ...
            if not target_player.is_alive:
                await EmbedHelper.send(interaction, description="❌ هذا اللاعب تم طرده بالفعل!", color=0xFF0000, is_ephemeral=True)
                return
            ...
            await EmbedHelper.send(interaction, description=f"✅ {interaction.user.mention} قام بطرد {target_player.user_name}!", color=0x00FF00, is_ephemeral=True)
            ...
            logger.error(f"❌ خطأ في طرد لاعب: {e}")
    
    async def auto_kick(self):
        try:
            if not self.game_active or not self.waiting_for_action:
                return
            ...
            logger.error(f"❌ خطأ في الطرد التلقائي: {e}")
    ...
    async def random_kick_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not alive_players:
                await EmbedHelper.send(interaction, description="❌ لا يوجد لاعبين آخرين للطرد!", color=0xFF0000, is_ephemeral=True)
                return
            
            target = random.choice(alive_players)
            await self.kick_player(interaction, target, current_player)
        except Exception as e:
            logger.error(f"❌ خطأ في زر الطرد العشوائي: {e}")
    ...
    async def kick_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            logger.error(f"❌ خطأ في زر طرد لاعب: {e}")
    ...
    async def select_callback(self, interaction: discord.Interaction):
        try:
            ...
            if target:
                ...
                await EmbedHelper.send(interaction, description="❌ حدث خطأ في اختيار اللاعب!", color=0xFF0000, is_ephemeral=True)
        except Exception as e:
            logger.error(f"❌ خطأ في اختيار اللاعب: {e}")
...
class VerifyView(discord.ui.View):
    ...
    async def verify_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not role:
                await EmbedHelper.send(interaction, description="❌ رتبة التحقق غير موجودة! يرجى إبلاغ الإدارة.", color=0xFF0000, is_ephemeral=True)
                return
            
            if role in interaction.user.roles:
                await EmbedHelper.send(interaction, description="✅ أنت بالفعل عضو موثق!", color=0x00FF00, is_ephemeral=True)
                return
            
            await interaction.user.add_roles(role)
            await EmbedHelper.send(interaction, description="✅ تم التحقق بنجاح! مرحباً بك في السيرفر.", color=0x00FF00, is_ephemeral=True)
...
class Games(commands.Cog):
    ...
    async def game_command(self, interaction: discord.Interaction):
        try:
            if interaction.guild_id in self.active_games:
                await EmbedHelper.send(interaction, description="❌ هناك لعبة نشطة بالفعل في هذا السيرفر!", color=0xFF0000, is_ephemeral=True)
                return
            ...
            logger.error(f"❌ خطأ في أمر game: {e}")
    
    async def start_game_session(self, interaction, game_name, game_type):
        try:
            if interaction.guild_id in self.active_games:
                await EmbedHelper.send(interaction, description="❌ هناك لعبة نشطة بالفعل!", color=0xFF0000, is_ephemeral=True)
                return
            
            game_data = self.get_game_data(game_type)
            if not game_data:
                await EmbedHelper.send(interaction, description="❌ لعبة غير معروفة!", color=0xFF0000, is_ephemeral=True)
                return
            ...
            logger.error(f"❌ خطأ في بدء جلسة لعبة: {e}")
    ...
    def create_game_listener(self, guild_id, game_type):
        async def on_message(message):
            try:
                ...
                logger.error(f"❌ خطأ في مستمع اللعبة: {e}")
    ...
    async def declare_winner(self, message):
        try:
            ...
            logger.error(f"❌ خطأ في إعلان الفائز: {e}")
    ...
    def get_game_data(self, game_type):
        games = {
            "speed_typing": {"description": "اكتب النص المعروض بأسرع وقت ممكن! أول من يكتبه بشكل صحيح يفوز.", "color": 0x2ECC71, "image_url": صورة_الروليت_الانتظار, "timeout": 30, "rules": "اكتب النص بالضبط كما يظهر."},
            "unscramble": {"description": "أعد ترتيب الحروف لتكوين كلمة صحيحة!", "color": 0xE74C3C, "image_url": صورة_الروليت_النتيجة, "timeout": 30, "rules": "أرسل الكلمة الصحيحة."},
            "math": {"description": "أجب عن المسألة الحسابية بسرعة!", "color": 0x3498DB, "image_url": صورة_الروليت_الانتظار, "timeout": 20, "rules": "أرسل الرقم الصحيح."},
            "trivia": {"description": "أجب عن سؤال المعلومات العامة!", "color": 0xF39C12, "image_url": صورة_الروليت_النتيجة, "timeout": 30, "rules": "أرسل رقم الخيار الصحيح (1، 2، 3، 4)."},
            "word_chain": {"description": "اكتب كلمة تبدأ بآخر حرف من الكلمة المعطاة!", "color": 0x9B59B6, "image_url": صورة_الروليت_الانتظار, "timeout": 30, "rules": "يجب أن تبدأ الكلمة بالحرف الأخير."},
            "emoji_guess": {"description": "خمن الكلمة أو العبارة من الإيموجي!", "color": 0x1ABC9C, "image_url": صورة_الروليت_النتيجة, "timeout": 30, "rules": "أرسل الكلمة الصحيحة."},
            "fill_blank": {"description": "أكمل الجملة بالكلمة الناقصة!", "color": 0xE67E22, "image_url": صورة_الروليت_الانتظار, "timeout": 25, "rules": "أرسل الكلمة الناقصة."},
            "reverse_word": {"description": "اقرأ الكلمة المعكوسة واكتبها بشكل صحيح!", "color": 0xC0392B, "image_url": صورة_الروليت_النتيجة, "timeout": 30, "rules": "اكتب الكلمة بالترتيب الصحيح."},
            "categories": {"description": "اكتب كلمة تنتمي للفئة المطلوبة!", "color": 0x27AE60, "image_url": صورة_الروليت_الانتظار, "timeout": 30, "rules": "يجب أن تنتمي الكلمة للفئة."},
            "first_to_type": {"description": "أول من يكتب أي شيء يفوز!", "color": 0x8E44AD, "image_url": صورة_الروليت_النتيجة, "timeout": 20, "rules": "أي رسالة تعتبر فوزاً."}
        }
    ...
    async def prepare_game_challenge(self, guild_id, game_type, channel):
        try:
            if game_type == "speed_typing":
                ...
                questions = [{"q": "ما هو أكبر كوكب في المجموعة الشمسية؟", "options": ["1. الأرض", "2. المشتري", "3. زحل", "4. المريخ"], "answer": "2"}, {"q": "كم عدد أيام السنة الميلادية؟", "options": ["1. 360", "2. 365", "3. 366", "4. 364"], "answer": "2"}, {"q": "ما هو لون السماء في النهار؟", "options": ["1. أحمر", "2. أزرق", "3. أخضر", "4. أصفر"], "answer": "2"}]
                ...
                challenges = [{"emojis": "🏠🐶", "answer": "بيت الكلب"}, {"emojis": "📚🏫", "answer": "مكتبة المدرسة"}, {"emojis": "🍎🌳", "answer": "شجرة التفاح"}]
                c = random.choice(challenges)
                self.active_games[guild_id]["correct_answer"] = c["answer"]
                await channel.send(f"😎 **خمن العبارة:**\n{c['emojis']}")
            elif game_type == "fill_blank":
                sentences = [{"text": "القاهرة عاصمة _____", "answer": "مصر"}, {"text": "الشمس تشرق من _____", "answer": "الشرق"}, {"text": "الماء _____ الحياة", "answer": "سر"}]
                ...
                categories = [{"name": "فواكه", "examples": "تفاح، برتقال، موز"}, {"name": "حيوانات", "examples": "أسد، نمر، فيل"}, {"name": "ألوان", "examples": "أحمر، أزرق، أخضر"}]
            ...
            logger.error(f"❌ خطأ في تحضير تحدي اللعبة: {e}")
...
class GameSelectionView(discord.ui.View):
    def __init__(self, games_cog):
        ...
        select = discord.ui.Select(placeholder="اختر لعبة...", options=options)
        select.callback = self.select_callback
        self.add_item(select)
    ...
    async def select_callback(self, interaction: discord.Interaction):
        try:
            ...
            game_names = {"speed_typing": "أسرع كاتب", "unscramble": "فكك الكلمة", "math": "الحساب السريع", "trivia": "معلومات عامة", "word_chain": "سلسلة الكلمات", "emoji_guess": "خمن الإيموجي", "fill_blank": "أكمل الفراغ", "reverse_word": "الكلمة المعكوسة", "categories": "فئات", "first_to_type": "أول من يكتب"}
            ...
            logger.error(f"❌ خطأ في اختيار اللعبة: {e}")
...
async def help_command(ctx):
    try:
        ...
        embed = EmbedHelper.create(
            ...
            color=0x3498db,
            fields=[{"name": "🎮 الألعاب", "value": "10 ألعاب سرعة ممتعة", "inline": True}, {"name": "🎡 الروليت", "value": "لعبة الروليت الجماعية", "inline": True}, {"name": "⚙️ الإدارة", "value": "أوامر إدارة السيرفر", "inline": True}],
            footer_text="استخدم /game لبدء الألعاب"
        )
...
async def roulette_command(ctx):
    try:
        ...
        logger.error(f"❌ خطأ في أمر الروليت: {e}")
...
async def roulette_slash(interaction: discord.Interaction):
    try:
        ...
        logger.error(f"❌ خطأ في أمر الروليت سلاش: {e}")
...
async def setup_verify(interaction: discord.Interaction, role: discord.Role, channel: discord.TextChannel):
    try:
        async with aiosqlite.connect("server_data.db") as db:
            await db.execute("INSERT OR REPLACE INTO server_config (guild_id, verify_role, verify_channel) VALUES (?, ?, ?)", (str(interaction.guild_id), str(role.id), str(channel.id)))
            await db.commit()
        
        view = VerifyView(role.id)
        embed = EmbedHelper.create(title="✅ نظام التحقق", description=f"اضغط على الزر أدناه للتحقق والحصول على رتبة {role.mention}", color=0x00FF00)
        await channel.send(embed=embed, view=view)
        await EmbedHelper.send(interaction, description=f"✅ تم إعداد نظام التحقق في {channel.mention} مع رتبة {role.mention}", color=0x00FF00, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في إعداد التحقق: {e}")
...
async def setup_welcome(interaction: discord.Interaction, channel: discord.TextChannel):
    try:
        async with aiosqlite.connect("server_data.db") as db:
            await db.execute("INSERT INTO server_config (guild_id, welcome_channel) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET welcome_channel = ?", (str(interaction.guild_id), str(channel.id), str(channel.id)))
            await db.commit()
        await EmbedHelper.send(interaction, description=f"✅ تم إعداد قناة الترحيب: {channel.mention}", color=0x00FF00, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في إعداد الترحيب: {e}")
...
async def setup_logs(interaction: discord.Interaction, channel: discord.TextChannel):
    try:
        async with aiosqlite.connect("server_data.db") as db:
            await db.execute("INSERT INTO server_config (guild_id, logs_channel) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET logs_channel = ?", (str(interaction.guild_id), str(channel.id), str(channel.id)))
            await db.commit()
        await EmbedHelper.send(interaction, description=f"✅ تم إعداد قناة السجلات: {channel.mention}", color=0x00FF00, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في إعداد السجلات: {e}")
...
async def setup_reports(interaction: discord.Interaction, channel: discord.TextChannel):
    try:
        async with aiosqlite.connect("server_data.db") as db:
            await db.execute("INSERT INTO server_config (guild_id, report_channel) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET report_channel = ?", (str(interaction.guild_id), str(channel.id), str(channel.id)))
            await db.commit()
        await EmbedHelper.send(interaction, description=f"✅ تم إعداد قناة البلاغات: {channel.mention}", color=0x00FF00, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في إعداد البلاغات: {e}")
...
async def block_links(interaction: discord.Interaction, status: str):
    try:
        async with aiosqlite.connect("server_data.db") as db:
            await db.execute("INSERT INTO server_config (guild_id, block_links) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET block_links = ?", (str(interaction.guild_id), int(status), int(status)))
            await db.commit()
        state = "تشغيل" if status == "1" else "إيقاف"
        await EmbedHelper.send(interaction, description=f"✅ تم {state} منع الروابط", color=0x00FF00, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في منع الروابط: {e}")
...
async def member_count(interaction: discord.Interaction, channel: discord.VoiceChannel):
    try:
        async with aiosqlite.connect("server_data.db") as db:
            await db.execute("INSERT INTO server_config (guild_id, member_count_channel) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET member_count_channel = ?", (str(interaction.guild_id), str(channel.id), str(channel.id)))
            await db.commit()
        member_count_val = len(interaction.guild.members)
        await channel.edit(name=f"👥 الأعضاء: {member_count_val}")
        await EmbedHelper.send(interaction, description=f"✅ تم إعداد عداد الأعضاء في: {channel.mention}", color=0x00FF00, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في إعداد عداد الأعضاء: {e}")
...
async def points_command(interaction: discord.Interaction):
    try:
        ...
        await EmbedHelper.send(interaction, title="📊 إحصائيات الألعاب", description=f"**{interaction.user.mention}**\n\n🏆 **النقاط:** {stats['points']}\n🎮 **الألعاب التي فاز بها:** {stats['games_won']}", color=0xFFD700, is_ephemeral=True)
    except Exception as e:
...
async def leaderboard_command(interaction: discord.Interaction):
    try:
        ...
        if not rows:
            await EmbedHelper.send(interaction, description="لا توجد بيانات بعد!", color=0xFFA500, is_ephemeral=True)
            return
        leaderboard_text = ""
        ...
        await EmbedHelper.send(interaction, title="🏆 لوحة صدارة الألعاب", description=leaderboard_text, color=0xFFD700, is_ephemeral=True)
    except Exception as e:
        logger.error(f"❌ خطأ في لوحة الصدارة: {e}")
...
async def on_ready():
    try:
        ...
        logger.info("✅ تم تحميل نظام الألعاب")
...
async def on_guild_join(guild):
    try:
        await تحديث_الدعوات(guild)
        logger.info(f"📥 انضم إلى سيرفر: {guild.name}")
    except Exception as e:
...
async def on_guild_remove(guild):
    try:
        ...
        logger.info(f"📤 غادر سيرفر: {guild.name}")
    except Exception as e:
        logger.error(f"❌ خطأ في on_guild_remove: {e}")
...
async def on_invite_create(invite):
    try:
        ...
        logger.error(f"❌ خطأ في on_invite_create: {e}")
...
async def on_invite_delete(invite):
    try:
        ...
        logger.error(f"❌ خطأ في on_invite_delete: {e}")
...
async def on_member_join(member):
    try:
        ...
        async with aiosqlite.connect("server_data.db") as db:
            today = datetime.now().strftime("%Y-%m-%d")
            await db.execute("INSERT INTO member_join_log (guild_id, date, count) VALUES (?, ?, 1) ON CONFLICT(guild_id, date) DO UPDATE SET count = count + 1", (str(guild.id), today))
            await db.commit()
        async with aiosqlite.connect("server_data.db") as db:
        ...
        logger.error(f"❌ خطأ في on_member_join: {e}")

@bot.event
async def on_member_remove(member):
    try:
        await تحديث_الدعوات(member.guild)
    except Exception as e:
        logger.error(f"❌ خطأ في on_member_remove: {e}")
...
async def on_message(message):
    try:
        ...
        logger.error(f"❌ خطأ في on_message: {e}")
