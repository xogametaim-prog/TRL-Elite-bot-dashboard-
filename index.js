...
const ROCKET_LEAGUE_QUESTIONS = [
    ...
    { question: "⚽ تكتيكياً: متى يجب عليك القيام بمهارة الـ Half-Flip للعودة للدفاع؟" },
    { question: "🎯 سؤال: كيف تضمن الفوز في الـ 50-50 في الكرات الأرضية؟" },
    { question: "🚀 ما رأيك في حركة الـ Ceiling Shot وهل تراها سهلة الدفاع؟" },
    { question: "🔥 تكتيك: هل تفضل البقاء خلف زميلك (Rotation) أم الضغط الثنائي الدائم؟" },
    { question: "💥 هل تعتمد على الشات السريع (Quick Chat) لتوجيه فريقك أم تفضل الصمت؟" },
    { name: "🚗 ما هو العشب المفضلة لديك في ملاعب روكيت ليق؟" },
    { name: "🏆 هل شاركت في بطولات روكيت ليق الرسمية داخل اللعبة (Tournaments)؟" },
    { name: "🌟 ما هو اللقب (Title) المفضل لديك المعلق تحت اسمك في اللعبة؟" },
    { name: "💨 هل تؤيد إلغاء ميزة الـ Trading الرسمية التي حدثت في اللعبة؟" },
    { name: "🌀 ما هي أفضل لقطة (Clip) قمت بتسجيلها في مسيرتك باللعبة؟" },
    { name: "🎮 هل تفضل اللعب مع الأصدقاء بالصوت (Voice Chat) أم اللعب الفردي التام؟" },
    { name: "🛡️ ما هو أفضل كوستومايز (تصميم سيارة) قمت بتركيبه حتى الآن؟" },
    { name: "🔥 هل تستخدم الـ Rocket Pass وهل يستحق الشراء دائماً؟" },
    { name: "⚽ كم مرة قمت بإنقاذ تاريخي في اللحظة الأخيرة (Epic Save) اليوم؟" },
    { name: "🏆 لو واجهت فريقاً محترفاً، ما هو التكتيك الذي ستعتمده للفوز؟" }
];

// قنوات ومفاتيح التحكم والتشغيل
...
let questionInterval = null;     // معرّف الإرسال التلقائي للأسئلة
const userWarns = new Map();     // لتتبع تكرار تخريب شات الصور
    ...
    return new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '⚽ ثالثاً: اختصارات أسئلة روكيت ليق التلقائية والعشوائية', value:
                `**-sn** : لتشغيل نظام البث والإرسال التلقائي للأسئلة بشكل عشوائي وممتع في الشات.\n` +
                `**-snp** : لإيقاف نظام الإرسال التلقائي للأسئلة العشوائية فوراً.\n` +
                `**-s** : لإرسال سؤال عشوائي وتكتيكي واحد فوراً في الشات لتجربة التفاعل والأعضاء.\n` +
...
// دالة توليد بطاقة ترحيبية مبتكرة باستخدام Canvas لسرعة فائقة (Skia Backend)
async function generateWelcomeImage(member) {
    ...
    try {
        ...
        // رسم دائرة افتراضية في حال تعطل جلب الصورة الشخصية للعضو
        ctx.fillStyle = '#5865F2';
        ctx.beginPath();
        ctx.arc(125, 125, 64, 0, Math.PI * 2, true);
        ctx.fill();
...
client.on('guildMemberAdd', async member => {
    // أ- منح الرتب التلقائية الفورية للأعضاء أو البوتات
    if (member.user.bot) {
        if (autoRoleBotId) {
    ...
    // ب- الترحيب بالصورة والاسم في الروم المخصص (-wel)
    if (welcomeChannelId) {
...
client.on('messageCreate', async message => {
    ...
    if (content.startsWith(CREATE_ROOM_PREFIX)) {
        if (!message.member.permissions.has(PermissionFlagsBits.ManageChannels)) {
            return message.reply('❌ عذراً، يجب أن تمتلك صلاحية إدارة القنوات لاستخدام هذا الأمر.');
        }
        ...
        try {
            // جلب صلاحيات الـ everyone الحالية من الروم الحالي لتطبيقها بدقة دون غيرها من رولات
            ...
            console.error(err);
            message.reply('❌ حدث خطأ أثناء محاولة إنشاء القناة، تأكد من صلاحيات البوت الإدارية.');
        }
        return;
    }

    // 3. أمر إضافة رتب لقناة نصية محددة
    ...
    // 4. تعيين قنوات السيرفر المختلفة بالمنشن والاختصارات
    ...
    // 5. تعيين الرتب التلقائية الفورية بدقة
    ...
    // 6. أمر تحويل الروم إلى (روم صور فقط) -puc
    ...
    // 7. تشغيل البث والأسئلة التلقائية التفاعلية عن روكيت ليق (-sn / -snp / -s)
    if (content === '-sn') {
        if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) return;
        if (questionInterval) {
        ...
        // إرسال أسئلة عشوائية كل 10 دقائق (600,000 مللي ثانية) بشكل مستمر
        questionInterval = setInterval(async () => {
            const randomItem = ROCKET_LEAGUE_QUESTIONS[Math.floor(Math.random() * ROCKET_LEAGUE_QUESTIONS.length)];
            const questionText = typeof randomItem === 'string' ? randomItem : (randomItem.question || randomItem.name);
    ...
    if (content === '-s') {
        // إرسال سؤال عشوائي فوري لمرة واحدة فقط لاختبار التفاعل
        const randomItem = ROCKET_LEAGUE_QUESTIONS[Math.floor(Math.random() * ROCKET_LEAGUE_QUESTIONS.length)];
        const questionText = typeof randomItem === 'string' ? randomItem : (randomItem.question || randomItem.name);
    ...
    // 8. حماية وإدارة روم الصور فقط (مسح نصوص وتنبيه العضو واللوج التلقائي للتخريب)
    if (picOnlyChannelId && message.channel.id === picOnlyChannelId) {
        // التحقق من أن الرسالة لا تحتوي على صور أو مرفقات
        ...
        if (!hasAttachment && !hasEmbedImage) {
            ...
            if (newWarns >= 5) {
                ...
                userWarns.set(message.author.id, 0); // تصفير عداد المخالفات بعد كتابة اللوج
            }
        }
    }
});
